...
if not ray.is_initialized():
    ray.init(
        num_cpus=4,  # Adjust based on your system's resources
        num_gpus=0,
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(log_dir, f'spline_quantile_transformer_tune_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
            encoding='utf-8'
        ),
...
def custom_trial_dirname_creator(trial: tune.Trial) -> str:
    """
    Create a custom directory name for each Ray Tune trial.

    Args:
        trial: A Ray Tune Trial object.

    Returns:
        A formatted string for the trial directory name.
    """
...
class SplineQuantileRegressionModel:
    ...
    def reshape_for_transformer(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for i in range(n_samples - self.time_steps + 1):
            X_window = X[i:i + self.time_steps, :]  # shape (self.time_steps, num_features)
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray, validation_data: Tuple[np.ndarray, np.ndarray], checkpoint_dir: Optional[str] = None):
        ...
        try:
            ...
            X_val_reshaped, y_val_train, valid_indices_val = self.reshape_for_transformer(X_val, y_val)
    ...
    def plot_results(self, y_true: np.ndarray, predictions: np.ndarray, title: str = 'Predictions', save_dir: str = None, scaler=None):
        try:
            if save_dir is None:
                save_dir = os.path.join(r'C:\Users\masoud\results\P\sqp', f'run_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
            os.makedirs(save_dir, exist_ok=True)
            if scaler is not None:
                y_true_original = scaler.inverse_transform(y_true.reshape(-1, 1)).flatten()
                predictions_original = scaler.inverse_transform(predictions.T).T
            ...
            plt.title(f'{title} - Spline Quantile Predictions', fontsize=14, fontweight='bold')
...
def tune_model(config: Dict, checkpoint_dir: Optional[str] = None, naive_error_train: Optional[float] = None):
    ...
    try:
        ...
        int_params = ['batch_size', 'epochs', 'num_samples', 'time_steps', 'num_knots',
                      'es_patience', 'rlr_patience', 'rlr_cooldown', 'num_encoder_layers']
...
def main():
    ...
    test_target = r"C:\Users\masoud\lreturns30w_test_scaled.xlsx"
    scaler_path = r"C:\Users\masoud\robust_scaler.pkl"
    EXPERIMENT_NAME = "sqp_tuning_campaign_v1"
    ...
    try:
        ...
        ray_results_dir = os.path.join(os.path.expanduser("~"), "ray_results_sqp_transformer")
        ...
        if best_trial:
            ...
            base_save_dir_plots = r'C:\Users\masoud\results\P\sqp_transformer_final'
            ...
            if final_model.model:
                final_model_path_to_save_final = os.path.join(final_model_plot_save_dir_path, 'sqp_transformer_model_final.keras')
